...
Collectibles = {
	...
	BLESSINGOTDEAD = Isaac.GetItemIdByName("Blessing of the Dead")
}
...
Curses = { --curses are bitflag oriented
	BLESSVISION = 1 << (Isaac.GetCurseIdByName("Blessing of the Vision") - 1),
	BLESSLIGHT = 1 << (Isaac.GetCurseIdByName("Blessing of the Light") - 1),
	BLESSFOUND = 1 << (Isaac.GetCurseIdByName("Blessing of the Found") - 1),
	BLESSCLARITY = 1 << (Isaac.GetCurseIdByName("Blessing of the Clarity") - 1),
	BLESSKNOWLEDGE = 1 << (Isaac.GetCurseIdByName("Blessing of the Knowledge") - 1)
}
...
--Function to shorten Bless o t dead code
--pretty messy overall
local function UpdateToBlesses()
	local player = Isaac.GetPlayer(0)
	local level = game:GetLevel()
	local levelCurses = level:GetCurses() --int as bitflags
	local tempEffects = player:GetEffects()
	
	if levelCurses & LevelCurse.CURSE_OF_DARKNESS ~= 0 then
		level:RemoveCurses(LevelCurse.CURSE_OF_DARKNESS)
		level:AddCurse(Curses.BLESSLIGHT, false)
	end
	if levelCurses & LevelCurse.CURSE_OF_THE_LOST ~= 0 then
		level:RemoveCurses(LevelCurse.CURSE_OF_THE_LOST)
		level:AddCurse(Curses.BLESSFOUND, false)
	end
	if levelCurses & LevelCurse.CURSE_OF_THE_UNKNOWN ~= 0 then
		level:RemoveCurses(LevelCurse.CURSE_OF_THE_UNKNOWN)
		level:AddCurse(Curses.BLESSKNOWLEDGE, false)
	end
	if levelCurses & LevelCurse.CURSE_OF_MAZE ~= 0 then
		level:RemoveCurses(LevelCurse.CURSE_OF_MAZE)
		level:AddCurse(Curses.BLESSCLARITY, false)
	end
	if levelCurses & LevelCurse.CURSE_OF_BLIND ~= 0 then
		level:RemoveCurses(LevelCurse.CURSE_OF_BLIND)
		level:AddCurse(Curses.BLESSVISION, false)
	end
	levelCurses = level:GetCurses()
	--works till here
	
	if levelCurses & Curses.BLESSCLARITY ~= 0 then
		level:ApplyCompassEffect(true)
	end	
	if levelCurses & Curses.BLESSFOUND ~= 0 then
		level:ApplyMapEffect()
	end
	--works flawless
	
	Isaac.DebugString(tostring(CustomData.Items.BLESSINGOTDEAD.BlessKnowledge))
	Isaac.DebugString(tostring(CustomData.Items.BLESSINGOTDEAD.BlessLight))
	Isaac.DebugString(tostring(CustomData.Items.BLESSINGOTDEAD.BlessVision))
		
	if levelCurses & Curses.BLESSKNOWLEDGE ~= 0 then
		if player:HasFullHearts() then
			player:AddSoulHearts(2)
		else
			player:AddHearts(24)
		end
		tempEffects:AddCollectibleEffect(CollectibleType.COLLECTIBLE_WAFER, false)
		CustomData.Items.BLESSINGOTDEAD.BlessKnowledge = true
	end
	if levelCurses & Curses.BLESSLIGHT ~= 0 then
		tempEffects:AddCollectibleEffect(CollectibleType.COLLECTIBLE_CAMO_UNDIES, false)
		CustomData.Items.BLESSINGOTDEAD.BlessLight = true
	end
	if levelCurses & Curses.BLESSVISION ~= 0 then
		tempEffects:AddCollectibleEffect(CollectibleType.COLLECTIBLE_THERES_OPTIONS, false)
		tempEffects:AddCollectibleEffect(CollectibleType.COLLECTIBLE_MORE_OPTIONS, false)
		CustomData.Items.BLESSINGOTDEAD.BlessVision = true
	end
	Isaac.DebugString(tostring(CustomData.Items.BLESSINGOTDEAD.BlessKnowledge))
	Isaac.DebugString(tostring(CustomData.Items.BLESSINGOTDEAD.BlessLight))
	Isaac.DebugString(tostring(CustomData.Items.BLESSINGOTDEAD.BlessVision))
	...
	if not Continued then
		CustomData = {
			Items = {
				...
				BLESSINGOTDEAD = {BlessVision = false, BlessLight = false, BlessKnowledge = false}
			},
			Cards = {
				REVERSECARD = nil,
...
function rplus:OnNewLevel()
	...
	if CustomData then
		CustomData.Items.BLESSINGOTDEAD.BlessLight = false
		CustomData.Items.BLESSINGOTDEAD.BlessVision = false
		CustomData.Items.BLESSINGOTDEAD.BlessKnowledge = false
		CustomData.Cards.JACK = nil
		CustomData.Items.CEILINGSTARS.SleptInBed = false
	end
	
	if player:HasCollectible(Collectibles.BLESSINGOTDEAD) then
		UpdateToBlesses()
	end
...
function rplus:OnNewRoom()
	...
	if CustomData and CustomData.Items.BLESSINGOTDEAD.BlessVision then
		tempEffects:AddCollectibleEffect(CollectibleType.COLLECTIBLE_MORE_OPTIONS, false)
		tempEffects:AddCollectibleEffect(CollectibleType.COLLECTIBLE_THERES_OPTIONS, false)
	end
	if CustomData and CustomData.Items.BLESSINGOTDEAD.BlessKnowledge then
		tempEffects:AddCollectibleEffect(CollectibleType.COLLECTIBLE_WAFER, false)
	end
	if CustomData and CustomData.Items.BLESSINGOTDEAD.BlessLight then
		tempEffects:AddCollectibleEffect(CollectibleType.COLLECTIBLE_CAMO_UNDIES, false)
	end
